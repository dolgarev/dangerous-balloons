...
procedure Main is
   ...
   procedure Draw_Splash is
      ...
      begin
         if Target_Row >= 0 and then Target_Row < Line_Position (L_S) and then Col_A >= 0 then
   ...
   begin
      ...
      Col_A := Column_Position (if Integer (C_S) > Art_W 
                                 then (Integer (C_S) - Art_W) / 2 
                                 else 0);
   ...
   begin
      if Quit_Requested then return; end if;
      ...
      loop
         ...
         Set_Character_Attributes (Standard_Window, Color => Color_Player_ID);
         Center_Text (Row + 2, "Choose starting level (1-" & Pad (Settings.Max_Levels, 2) & ")");

         Set_Character_Attributes (Standard_Window, Color => Color_Dead_ID);
         Center_Text (Row + 4, ">>>  Level " & Pad (Current_Selection, 2) & "  <<<");

         Set_Character_Attributes (Standard_Window, Color => Color_Status_ID);
         Center_Text (Row + 6, "Arrows/WASD - Select   SPACE/ENTER - Confirm   Q - Quit");
         ...
         if Key /= Terminal_Interface.Curses_Constants.ERR then
            case Key is
               when Key_Cursor_Up | Key_Cursor_Right | Character'Pos('w') | Character'Pos('W') | Character'Pos('d') | Character'Pos('D') =>
                  if Current_Selection < Settings.Max_Levels then
                     Current_Selection := Current_Selection + 1;
                  else
                     Current_Selection := 1;
                  end if;
               when Key_Cursor_Down | Key_Cursor_Left | Character'Pos('s') | Character'Pos('S') | Character'Pos('a') | Character'Pos('A') =>
                  if Current_Selection > 1 then
                     Current_Selection := Current_Selection - 1;
                  else
                     Current_Selection := Settings.Max_Levels;
                  end if;
               when Character'Pos (' ') | Character'Pos (ASCII.LF) | Character'Pos (ASCII.CR) =>
   ...
   procedure Spawn_Balloons is
      Placed : Integer := 0;
      R, C_Idx : Integer;
      Max_Spawn : constant Integer := (Settings.Initial_Balloons - 1) + Current_Level;
   begin
   ...
   procedure Move_Balloons is
      function Try_Move_Towards (B : in out Balloon_Rec) return Boolean is
         ...
         DC : Integer := 0;
      begin
         if Player_Dead then return False; end if;
...
begin
   ...
   exception
      when E : others =>
         Ada.Text_IO.Put_Line ("Failed to initialize screen: " & Exception_Message (E));
         return;
   end;
   ...
   loop
      ...
      loop
         ...
         begin
            ...
            declare
               L_Off : Line_Position := Line_Position (Integer (L_Count) - (Maze.Max_Rows + 2)) / 2;
               C_Off : Column_Position := Column_Position (Integer (C_Count) - Maze.Max_Cols) / 2;
            begin
               ...
               for R in 1 .. Maze.Max_Rows loop
                  for C_Idx in 1 .. Maze.Max_Cols loop
                     if (Integer (L_Off) + (R - 1) < Integer (L_Count)) and then (Integer (C_Off) + (C_Idx - 1) < Integer (C_Count)) then
                        Move_Cursor (Standard_Window, L_Off + Line_Position (R - 1), C_Off + Column_Position (C_Idx - 1));
                        case Grid (R, C_Idx) is
                           when Maze.Wall =>
                              Set_Character_Attributes (Standard_Window, Color => Color_Wall_ID);
                              Add (Standard_Window, String'[1 => Settings.Char_Wall]);
                           when Maze.Brick =>
                              Set_Character_Attributes (Standard_Window, Color => Color_Brick_ID);
                              Add (Standard_Window, String'[1 => Settings.Char_Brick]);
                           when Maze.Empty =>
               ...
               for I in Bombs'Range loop
                  if Bombs (I).Active then
                     if Bombs (I).Explosion_Tick > 0 then
                        ...
                        if (Integer (L_Off) + (Bombs (I).Row - 1) < Integer (L_Count)) and then (Integer (C_Off) + (Bombs (I).Col - 1) < Integer (C_Count)) then
                           Move_Cursor (Standard_Window, L_Off + Line_Position (Bombs (I).Row - 1), C_Off + Column_Position (Bombs (I).Col - 1));
                           Set_Character_Attributes (Standard_Window, Color => Color_Bomb_ID);
                           declare
                              C_Num : constant Character := Character'Val (Character'Pos ('0') + Bombs (I).Timer);
               ...
               for I in Balloons'Range loop
                  if Balloons (I).Active then
                     if (Integer (L_Off) + (Balloons (I).Row - 1) < Integer (L_Count)) and then (Integer (C_Off) + (Balloons (I).Col - 1) < Integer (C_Count)) then
                        Move_Cursor (Standard_Window, L_Off + Line_Position (Balloons (I).Row - 1), C_Off + Column_Position (Balloons (I).Col - 1));
                        if Balloons (I).Death_Tick > 0 then
                           Set_Character_Attributes (Standard_Window, Attr => (Blink => True, others => False), Color => Color_Balloon_D_ID);
                        elsif Global_Balloon_Mode = Friendly then
                           Set_Character_Attributes (Standard_Window, Color => Color_Balloon_F_ID);
                        else
                           Set_Character_Attributes (Standard_Window, Color => Color_Balloon_H_ID);
                        end if;
                        Add (Standard_Window, String'[1 => Settings.Char_Balloon]);
                     end if;
                  end if;
               end loop;

               if (Integer (L_Off) + (Player_Row - 1) < Integer (L_Count)) and then (Integer (C_Off) + (Player_Col - 1) < Integer (C_Count)) then
                  if Player_Dead then
                     if Player_Death_Tick > 0 then
                        Move_Cursor (Standard_Window, L_Off + Line_Position (Player_Row - 1), C_Off + Column_Position (Player_Col - 1));
                        Set_Character_Attributes (Standard_Window, Attr => (Blink => True, others => False), Color => Color_Dead_ID);
                        Add (Standard_Window, String'[1 => Settings.Char_Dead_Player]);
                     end if;
                  else
                     Move_Cursor (Standard_Window, L_Off + Line_Position (Player_Row - 1), C_Off + Column_Position (Player_Col - 1));
                     Set_Character_Attributes (Standard_Window, Color => Color_Player_ID);
               ...
               declare
                  Status    : constant String := " LEVEL: " & Pad (Current_Level, 2) & 
                                                 "   SCORE: " & Pad (Score, 5) & 
                                                 "   LIVES: " & Pad (Lives, 2) & " ";
               ...
               if (Player_Dead and then (Player_Death_Tick = 0 and then Lives = 0)) then
                  declare
                     M1 : constant String := "GAME OVER";
                     M2 : constant String := "PRESS ANY KEY TO CONTINUE";
                     R1 : constant Line_Position := L_Off + Line_Position (Maze.Max_Rows / 2);
                     R2 : constant Line_Position := R1 + 1;
                     C1 : constant Column_Position := C_Off + Column_Position ((Maze.Max_Cols - M1'Length) / 2);
                     C2 : constant Column_Position := C_Off + Column_Position ((Maze.Max_Cols - M2'Length) / 2);
                  begin
                     Set_Character_Attributes (Standard_Window, Color => Color_Dead_ID, Attr => (Blink => True, others => False));
                     Move_Cursor (Standard_Window, R1, C1);
                     Add (Standard_Window, M1);
                     Move_Cursor (Standard_Window, R2, C2);
         ...
         elsif (Player_Dead and then (Player_Death_Tick = 0 and then Lives = 0)) then
            if Ch /= Terminal_Interface.Curses_Constants.ERR then
               exit Game_Loop;
            end if;
         elsif (not Player_Dead) and then (Ch /= Terminal_Interface.Curses_Constants.ERR) then
                  ...
                  if not Bombs (I).Active then
                     ...
                     begin
                        for J in Bombs'Range loop
                           if Bombs (J).Active and then (Bombs (J).Row = Player_Row) and then (Bombs (J).Col = Player_Col) then
                              Exists := True;
                           end if;
                        end loop;
                        if not Exists then
...
exception
   when E : others =>
      End_Windows;
      Ada.Text_IO.Put_Line ("Unexpected error in main loop: " & Exception_Name (E) & " - " & Exception_Message (E));
      raise;
end Main;
